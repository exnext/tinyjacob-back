...
export class AppService {
  ...
  async probabilityMessage(params): Promise<string> {
    ...
    return `${params.name} probability of you living in ${new Intl.DisplayNames(['EN'], { type: 'region' }).of((await json).country[0].country_id)} is ${(await json).country[0].probability * 100} %`;
  }
  bToKb(size) {
